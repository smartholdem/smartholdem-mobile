<template>
  ...
  <div class="row">
      ...
      <card class="radius-0 mt-0">
        <div class="mb-3">
        <span class="badge badge-success big-badge mr-3">SmartHolders <span class="font-weight-bolder p-2">{{holders.total.holders}}</span></span>
        <span class="badge badge-success big-badge">Total Hold <span class="font-weight-bolder p-2">{{(holders.total.realAmount * 1).toFixed(0)}}</span> STH</span>
          <span class="btn btn-outline-warning btn-simple btn-sm float-right pointer"
             @click="navUrl('https://community.smartholdem.io/topic/530/staking-at-xbts-up-to-50-of-trading-profit')"
             >
            Join Now
          </span>
          <p>
            The <strong>SmartHOLDER Staking Program</strong> gives benefits to the STH owners. Investing in SmartHoldem Coin, you have the right for a share of earnings 50% of the XBTS DEX trading proceeds, which are generated by trading fees on the XBTS exchange. Increase your crypto portfolio regardless of how the market moves.
          </p>
          <p>
            You can place your SmartHoldem coins STH and get profit from XBTS twice a month. The stake in the SmartHOLDER Staking Program will freeze and block your coins in the blockchain for the timer of 1-3-6-12 months. The longer duration of the stake provides for the higher pay-out rate. During the staking period you will automatically take part in the distribution of payments on the 1st and 15th day of each month.
          </p>
        </div>
        ...
        <div class="card-body">
          <table class="table mb-0 table-stripe table-responsive">
            ...
            <tr v-for="(item,idxNoq) in assetsNoq" :key="idxNoq" class="pointer">
              <td class="text-left"><img width="24px" :src="'https://ex.xbts.io/asset-symbols/'+item.ticker.toLowerCase()+'.png'"
                                         class="asset-symbol mr-2"/>
              </td>
