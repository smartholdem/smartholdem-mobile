<template>
  <div>
    <div class="row">
      <div class="col-md-12">
        <card class="bg-white" style="height: 86vh;overflow: hidden;margin-bottom: 0px;">
          ...
          <template slot="header" class="">
            <div class="row border-bottom">
              ...
              <div class="col-md-4 pl-2 text-center">
                ...
                <h4 class="card-title font-weight-normal"><span class="font-weight-bold">{{ walletBalance.balance }}</span>
                  STH</h4>

                <h4 title="Your Public Address" class="card-title font-weight-bold" v-clipboard="() => currentAddress">
                  <i class="tim-icons icon-single-copy-04 pointer"></i>
                  {{currentAddress.substr(0,8)}}..{{currentAddress.substr(-8)}}
                </h4>
                ...
                <h4 v-if="accountData.delegate" class="card-title">
                  ...
                  &nbsp;<span v-if="accountData.vote" @click="showModalVote({address: accountData.address, delegate: accountData.vote, voteType: '-'})"
              ...
              <div class="col-md-7 pl-4">
                <base-button @click="showModalSend(currentAddress)" :disabled="accountData.balance < 1.01" type="primary" round simple>
                  <el-tooltip
                ...
                <base-button v-if="!accountData.delegate" type="default" :disabled="accountData.balance < 55000" @click="showModalDelegate" simple round class="ml-2 text-uppercase" style="">
                  <i class="tim-icons icon-bank pb-1" style="font-size: 0.96rem"></i> Register Delegate
                </base-button>

                <base-button v-if="!accountData.delegate" @click="showModalLabel($route.params.address)" type="info" round icon
                             simple class="ml-2">
    ...
    <ModalTxDelegate :showModalDelegate="modal.delegate.show" :address="modal.delegate.address" @onModalClose="modal.delegate.show = false"/>
    </div>
